...
const fetchGames = async ({ pageParam }: { pageParam: number }) => {
    ...
    //     },
    // }).then(res => res.data);
    return axios.get(`https://api.rawg.io/api/games?key=c542e67aec3a4340908f9de9e86038af&page=${pageParam}&page_size=40`).then(res => res.data)
      ...
      <div className="flex flex-col">
           {data?.pages.map((page, i) => (
               ...
               {page.results.map((game: IGameCard) => (
                 <>
                   <GameCard key={game.id} id={game.id} slug={game.slug} img={game.background_image} name={game.name} genres={game.genres}  />
                 </>
